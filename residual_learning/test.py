#!/usr/bin/env python3
"""
train_unified.py - V3: ROBUST VALIDATION

- FINAL UPDATE: The model is now validated against the **entire trajectory** periodically.
- The "best" model is saved based on its final positional error over the full simulation,
  ensuring that the selection criteria matches the final evaluation criteria.
- This prevents overfitting to a small validation segment.
...
def evaluate_simulation_performance(model, full_meas, full_ctrl, H, dt, device):
    """
    Runs a full closed-loop simulation on the ENTIRE trajectory and returns the final positional error.
    This function's logic now matches the final evaluation script.
    """
    model.eval()
    N = len(full_meas)
    sim_states = []
    state = full_meas[0].copy()
    ...
    with torch.no_grad():
        for k in range(N):
            ...
            current_features = feat_tensor(state, full_ctrl[k], device)
            ...
            next_state_base = simulate_step(state, full_ctrl[k], dt)
    ...
    real_pos_x = np.array([s['pos_x'] for s in full_meas])
    real_pos_y = np.array([s['pos_y'] for s in full_meas])
...
def plot_curves(train_history, val_history, val_epochs, model_type, encoder_type):
    ...
    color = 'tab:blue'
    ax1.set_xlabel('Epochs'); ax1.set_ylabel('Average Training MSE Loss', color=color)
    ax1.plot(epochs_train, train_history, 'b-o', label='Training MSE')
    ax1.tick_params(axis='y', labelcolor=color); ax1.grid(True)
    ax2 = ax1.twinx()
    color = 'tab:red'
    ax2.set_ylabel('Full Trajectory Positional Error [m]', color=color)
    ax2.plot(val_epochs, val_history, 'r-s', label='Full Sim Positional Error')
    ax2.tick_params(axis='y', labelcolor=color)
    fig.suptitle(f'Training & Validation Performance for {model_type.upper()} with {encoder_type.upper()} Encoder')
    fig.tight_layout()
    plt.savefig(f'training_curve_{model_type}_{encoder_type}_full_val.png')
    print(f"Saved training curve to training_curve_{model_type}_{encoder_type}_full_val.png")
    plt.show()

# ----------------------- DATASET & MODEL DEFS (Unchanged) -----------------------
...
class ResidualModel_Linear(torch.nn.Module):
    ...
    def forward(self, x): z = self.encoder(x); pred_norm = self.regressor(z); return pred_norm * self.y_std + self.y_mean
    def predict_normalized(self, x): z = self.encoder(x); return self.regressor(z)
...
def train_model(model, train_loader, full_meas, full_ctrl, dt, device, epochs, args):
    scheduler = CosineAnnealingWarmRestarts(model.opt, T_0=20, T_mult=2)
    best_pos_error = float('inf')
    save_path = Path(args.save)
    best_model_path = save_path.with_name(f"{save_path.stem}_{args.model_type}_{args.encoder}_best_full_sim{save_path.suffix}")
    train_mse_history, val_pos_error_history, val_epochs = [], [], []

    for ep in range(1, epochs + 1):
        model.train(); pbar = tqdm(train_loader, desc=f"Epoch {ep}/{epochs} [{args.model_type.upper()} Train]"); total_train_mse = 0.0
        for bx, by, idx in pbar:
            bx, by, i0s = bx.to(device), by.to(device), idx.numpy()
            model.opt.zero_grad()
            is_svgp = hasattr(model, 'mll')
            if is_svgp:
                dist_norm = model.gp(model.encoder(bx)); one_step_loss = -model.mll(dist_norm, by)
                pred_unnorm = model.lik(dist_norm).mean * model.y_std + model.y_mean
            else:
                pred_norm = model.predict_normalized(bx); one_step_loss = model.loss_fn(pred_norm, by)
        ...
        if ep % args.eval_freq == 0 or ep == epochs:
            pos_error = evaluate_simulation_performance(model, full_meas, full_ctrl, args.hist, dt, device)
            print(f"\n--- Full Sim Validation @ Epoch {ep} | Final Positional Error: {pos_error:.3f} m ---\n")
            val_pos_error_history.append(pos_error); val_epochs.append(ep)
            if pos_error < best_pos_error:
                best_pos_error = pos_error
                print(f"🎉 New best model! Full Sim Positional Error: {best_pos_error:.3f} m. Saving to {best_model_path}...")
                ...
                if is_svgp: save_payload.update({'gp': model.gp.state_dict(), 'lik': model.lik.state_dict()})
                else: save_payload['regressor'] = model.regressor.state_dict()
                torch.save(save_payload, best_model_path, _use_new_zipfile_serialization=False) # For compatibility with older torch versions
        scheduler.step()
    return train_mse_history, val_pos_error_history, val_epochs

# ----------------------- MAIN -----------------------
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Train a residual dynamics model, optimizing for full-simulation positional error.")
    ...
    # The model now trains on windows from the entire dataset
    X, Y = build_data(full_meas, full_ctrl, args.hist, dt)
    ...
    train_history, val_history, val_epochs = train_model(
        model, train_loader, full_meas, full_ctrl, dt, device, args.epochs, args
    )
    print("\nTraining complete. The best model (based on FULL SIMULATION positional error) has been saved.")
